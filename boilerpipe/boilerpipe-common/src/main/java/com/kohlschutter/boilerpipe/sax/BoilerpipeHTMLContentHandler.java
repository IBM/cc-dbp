...
public class BoilerpipeHTMLContentHandler implements ContentHandler {
  ...
  String baseUri;
  List<BPAnnotation> annotations = new ArrayList<>();
  ...
  public void addLinkStart(String href) {
      if (href == null)
          return;
      if (href.indexOf("://") != -1) {
          //nothing it is absolute
      } else if (href.startsWith("/")) {
          //TODO: path based on URI
      } else if (baseUri != null) {
          href = baseUri + href;
      } else {
          //TODO: relative to current uri
      }
      Link link = new Link(href.trim());
      addAnnotation(link);
  }
  public void addAnnotation(BPAnnotation anno) {
      anno.start = textBuffer.length();
      anno.end = -1;
      annotations.add(anno);
  }
  public void closeAnnotation(String localName) {
      localName = localName.toLowerCase();
      BPAnnotation lastTagOfType = null;
      for (int i = annotations.size()-1; i >= 0; --i) {
          if (annotations.get(i).localName.equals(localName)) {
              lastTagOfType = annotations.get(i);
              break;
          }
      }
      if (lastTagOfType != null) {
          if (lastTagOfType.end == -1)
              lastTagOfType.end = textBuffer.length();
          //else
          //    System.err.println("bad link end: "+textBuffer);
      }
  }
  ...
  public void startElement(String uri, String localName, String qName, Attributes atts)
      throws SAXException {
    //SparseVectors.increase(startTagCounts, localName, 1.0);
    labelStacks.add(null);
    
    if (inBody == 0 && localName.equalsIgnoreCase("base")) {
        //use the baseUri for relative links
    ...
    if (localName.equalsIgnoreCase("div") || localName.equalsIgnoreCase("ul") || localName.equalsIgnoreCase("li") || localName.equalsIgnoreCase("p") || localName.equalsIgnoreCase("br"))
        paragraphBreak();
  }

  //public static Map<String,MutableDouble> endTagCounts = new HashMap<>();
  //public static Map<String,MutableDouble> startTagCounts = new HashMap<>();
  
  // @Override
  public void endElement(String uri, String localName, String qName) throws SAXException {
    //SparseVectors.increase(endTagCounts, localName, 1.0);
    TagAction ta = tagActions.get(localName);
    if (ta != null) {
    ...
    if (localName.equalsIgnoreCase("div") || localName.equalsIgnoreCase("ul") || localName.equalsIgnoreCase("li") || localName.equalsIgnoreCase("p"))
        paragraphBreak();
  }

  void paragraphBreak() {
      ParagraphAnnotation p = new ParagraphAnnotation();
      p.start = textBuffer.length();
      p.end = textBuffer.length()+1;
      annotations.add(p);
      /*
      char e = textBuffer.charAt(textBuffer.length()-1);
      if (Character.isWhitespace(e)) {
          textBuffer.setLength(textBuffer.length()-1);
          tokenBuffer.setLength(textBuffer.length()-1);
      }
      textBuffer.append('\n');
      tokenBuffer.append('\n');
  ...
  public void flushBlock() {
    ...
    switch (length) {
      ...
      case 1:
        if (sbLastWasWhitespace) {
          textBuffer.setLength(0);
          tokenBuffer.setLength(0);
          for (BPAnnotation a : annotations) {
              a.start = 0;
          }
          return;
        }
    ...
    annotations.removeIf(a -> !a.isValid());
    
    //mrglass: fix annotations while trimming whitespace
    int adjustStart = 0;
    while (adjustStart < textBuffer.length() && Character.isWhitespace(textBuffer.charAt(adjustStart)))
        ++adjustStart;
    int adjustEnd = textBuffer.length()-1;
    while (adjustEnd >= adjustStart && Character.isWhitespace(textBuffer.charAt(adjustEnd)))
        --adjustEnd;
    ++adjustEnd;
    for (BPAnnotation l : annotations) {
        if (l.end > adjustEnd)
            l.end = adjustEnd;
       l.addOffset(-adjustStart);
    }
    TextBlock tb =
        new TextBlock(textBuffer.substring(adjustStart, adjustEnd), currentContainedTextElements, numWords,
            numLinkedWords, numWordsInWrappedLines, numWrappedLines, offsetBlocks, annotations);
