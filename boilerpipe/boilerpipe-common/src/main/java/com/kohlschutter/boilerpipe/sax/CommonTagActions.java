...
public abstract class CommonTagActions {
  ...
  //TODO: could have table and list annotations too
  ...
  public static final TagAction TA_ANCHOR_TEXT = new TagAction() {
    ...
    public boolean start(BoilerpipeHTMLContentHandler instance, final String localName,
      ...
      if (instance.inAnchor++ > 0) {
        ...
        //mrglass: this is much less common with the fixes from boilerpipes shaded version, but still happens. We can just ignore it.
      ...
      if (instance.inIgnorableElement == 0) {
        ...
        //mrglass: mark instance.textBuffer start and add the href
        String href = atts.getValue("href");
        if (href != null)
            instance.addLinkStart(href);
    ...
    public boolean end(BoilerpipeHTMLContentHandler instance, final String localName,
      ...
      if (--instance.inAnchor == 0) {
        if (instance.inIgnorableElement == 0) {
          //mrglass: mark instance.textBuffer end for the link annotation
          instance.closeAnnotation("a");
          instance.addWhitespaceIfNecessary();
  ...
  public static final TagAction TA_BODY = new TagAction() {
    public boolean start(final BoilerpipeHTMLContentHandler instance, final String localName,
        final String qName, final Attributes atts) {
      instance.flushBlock();
      //TODO: if instance.baseUri is not set, set it according to the uri of the document
      instance.inBody++;
      instance.addAnnotation(new ParagraphAnnotation());
      return false;
    }
  ...
  public static final TagAction TA_INLINE_NO_WHITESPACE = new TagAction() {
    ...
    public boolean start(BoilerpipeHTMLContentHandler instance, final String localName,
        final String qName, final Attributes atts) {
        if (localName.equalsIgnoreCase("b") || localName.equalsIgnoreCase("em") || 
                localName.equalsIgnoreCase("strong") || localName.equalsIgnoreCase("i") ||
                localName.equalsIgnoreCase("u")) 
        {
            //add formatting annotation
            instance.addAnnotation(new TextFormatAnnotation(localName.toLowerCase()));
        }
      return false;
    }
    ...
    public boolean end(BoilerpipeHTMLContentHandler instance, final String localName,
        final String qName) {
        instance.closeAnnotation(localName);
      return false;
    }
  ...
  public static final class BlockTagLabelAction implements TagAction {
    ...
    public boolean start(BoilerpipeHTMLContentHandler instance, final String localName,
        final String qName, final Attributes atts) {
      instance.addLabelAction(action);
      if (localName.equalsIgnoreCase("h1") || localName.equalsIgnoreCase("h2") || 
              localName.equalsIgnoreCase("h3") || localName.equalsIgnoreCase("h4")) {
          instance.addAnnotation(new HeaderAnnotation(localName));
      }
      return true;
    }
