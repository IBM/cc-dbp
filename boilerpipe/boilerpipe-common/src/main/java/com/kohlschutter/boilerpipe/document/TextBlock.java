...
public class TextBlock implements Cloneable {
  ...
  //mrglass
  Collection<BPAnnotation> annotations = null;
  ...
  public static final TextBlock EMPTY_START = new TextBlock("", EMPTY_BITSET, 0, 0, 0, 0, -1, Collections.EMPTY_LIST);
  public static final TextBlock EMPTY_END = new TextBlock("", EMPTY_BITSET, 0, 0, 0, 0,
      Integer.MAX_VALUE, Collections.EMPTY_LIST);

  public TextBlock(final String text) {
    this(text, null, 0, 0, 0, 0, 0, Collections.EMPTY_LIST);
  }
  ...
  public TextBlock(final String text, final BitSet containedTextElements, final int numWords,
      ...
      final int offsetBlocks, final Collection<BPAnnotation> annos) {
    this.text = text;
    ...
    this.annotations = new ArrayList<>(annos);
    initDensities();
  }
  ...
  public void mergeNext(final TextBlock other) {
    ...
    sb.append('\n');
    int linkOffset = sb.length();
    sb.append(other.text);

    //mrglass: combine annotations from other
    for (BPAnnotation anno : other.annotations) {
        anno = anno.clone(); //CONSIDER: needed?
        anno.addOffset(linkOffset);
  ...
  public String toDebugString() {
      StringBuilder buf = new StringBuilder(
              "[" + offsetBlocksStart + "-" + offsetBlocksEnd + ";tl=" + tagLevel + "; nw=" + numWords
              + ";nwl=" + numWrappedLines + ";ld=" + linkDensity + "]\t"
              + (isContent ? "CONTENT" : "boilerplate") + "," + labels + "\n" + getText());
      for (BPAnnotation a : annotations) {
          buf.append("   "+a.localName+", "+a.start+" "+a.end);
      }
      return buf.toString();
  }
  ...
  protected TextBlock clone() {
    ...
    //mrglass: clone annotations
    if (annotations != null) {
        clone.annotations = new ArrayList<>();
        for (BPAnnotation anno : annotations)
            clone.annotations.add(anno.clone());
    }
